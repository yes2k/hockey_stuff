...
Mixed Effect Model, hierarchical model, multilevel models, they go by many names but they all essentially do the same thing, try to model data that has some grouping or nesting structure. It's an extension of linear regression that accounts for non independence in the data. In this case, we are looking at NHL shot data, so each observation of our data is a shot and various information about the shot like where the shot took place, the distance of the shot from the net, the player who took the shot, the goalie and much more. One of the ways we can group our data is according to the player that took the shot, since our data consists of players who have taken multiple shots. Since our data is also binary, we will be using a logistic mixed model which can be defined as follows:

$$ \text{isgoal}_i \thicksim \text{Bernoulli}(p_i)$$

```{=tex}
<!-- \begin{multline*} -->
\text{logit} \  p_i = \beta_0 + \beta_1 x_i + \beta_2 y_i + \beta_3\text{shot_distance}_i + \beta_4\text{shot_angle}_i + \\
\beta_5\text{seconds_since_last_shot}_i + \text{shooter}_i + \text{goalie}_i + \\ \text{event_detail}_i + \text{score_state}_i + \text{game_strength_state}_i 
<!-- \end{multline*} -->
```
$$
\beta_0, ..., \beta_{5} \thicksim \mathcal{N}(0, 1)
$$

<!-- \\begin{multline\*} shooter_i \thicksim \\mathcal{N}(0, \sigma\_{\text{shooter}}) \\begin{multline\*} -->
...
<!--                   "game_strength_state", "score_state", "game_seconds") %>% drop_na() -->
<!--   return(new_pbp)    -->
<!-- } -->
