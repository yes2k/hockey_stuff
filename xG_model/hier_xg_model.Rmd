...
Since our data is also binary, we will be using a logistic mixed model which can be defined as follows:
...
Since this is a Bayesian model, we need priors
$$
\beta_0, ..., \beta_{5} \thicksim \mathcal{N}(0, 1)
$$

The random effects will each have a $\mathcal{N}(0, \sigma)$ distribution and the prior for the $\sigma$ will be _________.


The random effects in our model are the shooter, goalie, event_detail (the type of shot), the score state and the game strength state.
